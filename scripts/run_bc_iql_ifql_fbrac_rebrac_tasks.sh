...
OUTPUT_DIR="${OUTPUT_DIR:-bc_iql_ifql_fql_fbrac_rebrac_result}"
SAVE_ROOT="${SAVE_ROOT:-${OUTPUT_DIR}/experiments}"
LOG_DIR="${LOG_DIR:-${OUTPUT_DIR}/logs}"
SEEDS_STR="${SEEDS:-1}"
TASK_IDS_STR="${TASK_IDS:-task1 task2 task3 task4 task5}"
AGENT_FILES_STR="${AGENT_FILES:-agents/bc.py agents/iql.py agents/ifql.py agents/fql.py agents/fbrac.py agents/rebrac.py}"
ENV_PREFIXES_STR="${ENV_PREFIXES:-cube-double-play scene-play puzzle-3x3-play}"
...
check_python() {
  ...
  if ! "$PYTHON_BIN" -c "import sys; print(sys.executable)" > "$preflight_log" 2>&1; then
    ...
    echo "Example:"
    echo "  PYTHON_BIN=/path/to/python bash scripts/run_bc_iql_ifql_fbrac_rebrac_tasks.sh"
...
echo "All BC/IQL/IFQL/FQL/FBRAC/ReBRAC non-visual task runs finished."
